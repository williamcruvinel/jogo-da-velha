...
function initializeGame() {
  ...
  boardRegions.forEach(function (element) {
    ...
    element.innerText = ''
    element.classList.remove('cursor-arrow')
    element.classList.add('cursor-pointer')
    element.addEventListener('click', handleBoardClick)

    document.querySelector('.result').innerHTML = ''
    document.querySelector('#start').innerText = "JOGAR"
  })
}
...
function disableRegion(element) {
  ...
  element.classList.add('cursor-arrow')
...
function handleWin(regions) {
  ...
  document.querySelector('.result').innerHTML = '<h3></h3>'
  document.querySelector('h3').innerHTML = playerName + " venceu!!!"
  document.querySelector('#start').innerText = "NOVO JOGO"
...
function handleBoardClick(ev){
  ...
  if (winRegions.length > 0) {
    ...
    document.querySelector('.result').innerHTML = '<h3> Deu empate!!! </h3>'
  }
}
